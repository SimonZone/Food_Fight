...
public class WordScript : MonoBehaviour
{
    ...
    public List<string> wordBankJSON;
    ...
    {
        string lettersToPaint = "";
        string lettersNotToPaint = "";

        for (int i = 0; i < currentWord.Length; i++)
        {
            if (i < currentIndex)
            {
                lettersToPaint += currentWord[i];
            } else if (i >= currentIndex)
            {
                lettersNotToPaint += currentWord[i];
            }
        }
        //Debug.Log("word that is being changed: " + letterToPaint + " - " + letterNotToPaint);

        wordUnderProjectile.SetText($"" +
            $"{lettersToPaint.AddColor(Color.green)}" +
            $"{lettersNotToPaint.AddColor(Color.red)}");
